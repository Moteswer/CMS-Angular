...
from .models import Staff, Admin, Doctor, Login, Patient, Role, Specialization, Appointment, Diagnosis, Medicine, Bill, \
    Test
from .serializers import SignupSerializer, LoginSerializer, StaffSerializer, DoctorSerializer, PatientSerializer, \
    ...
    DiagnosisSerializer, MedicineSerializer, TestSerializer
...
class DoctorPasswordUpdateView(APIView):
...
class GetDoctorIdByEmail(APIView):
    def get(self, request, email):
        try:
            doctor = Doctor.objects.get(email=email)
            return Response({'doctorId': doctor.doctor_id}, status=status.HTTP_200_OK)
        except Doctor.DoesNotExist:
            return Response({'error': 'Doctor not found for provided email'}, status=status.HTTP_404_NOT_FOUND)


@csrf_exempt
def post_diagnosis_data(request):
    if request.method == 'POST':
        try:
        ...
        serializer = DiagnosisSerializer(diagnosis_data, many=True)
        return JsonResponse(serializer.data, safe=False)
...
def medicine_list(request):
    """
    List all medicines or create a new Medicine instance.
    """
    if request.method == 'GET':
...
@csrf_exempt
def toggle_active_medicine(request, medicine_id):
    if request.method == 'PUT':
        doctor = get_object_or_404(Medicine, pk=medicine_id)
        doctor.is_active = not doctor.is_active
        doctor.save()
        return JsonResponse({'success': True})
    else:
...
def check_medicine_exists(request):
    if request.method == 'GET':
        medicine_name = request.GET.get('medicine_name', None)
        if medicine_name is not None:
            # Check if the medicine exists in the database (case-insensitive)
            medicine_exists = Medicine.objects.filter(medicine_name__iexact=medicine_name).exists()
            return JsonResponse({'exists': medicine_exists})
        else:
            return JsonResponse({'error': 'Medicine name not provided'}, status=400)
    else:
        return JsonResponse({'error': 'Only GET requests are allowed'}, status=405)

def check_medicine_exists_by_name(request, medicine_name):
    try:
        # Check if a medicine with the given name exists
...
def get_medicine_id_by_name(request, medicine_name):
    try:
        # Assuming Medicine model has a field called 'name' for the medicine name
        medicine = Medicine.objects.get(medicine_name=medicine_name)
        return JsonResponse({'medicine_id': medicine.medicine_id})
    except Medicine.DoesNotExist:
        return JsonResponse({'error': 'Medicine not found'}, status=404)

def check_medicine_exists_by_id(request, medicine_id):
    try:
        # Check if a medicine with the given ID exists
...
def get_bill(request):
    try:
        ...
        for bill in bills:
            bill_data = {
                'diagnosis_id': bill.diagnosis_id,
        ...
        return JsonResponse({'error': 'Bills not found'}, status=404)
...
def create_bill(request):
    if request.method == 'POST':
        try:
            ...
            return JsonResponse({'success': 'Bill created successfully'}, status=201)
        except json.JSONDecodeError:
            return JsonResponse({'error': 'Invalid JSON data'}, status=400)
...
@csrf_exempt
def toggle_active_test(request, pk):
    if request.method == 'PUT':
